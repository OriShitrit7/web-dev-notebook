...
```quiz
? איפה נכתב attribute?
+ בתוך תגית הפתיחה בלבד
- בתוך תגית הפתיחה ובתגית הסגירה
- בין תגית הפתיחה לתגית הסגירה
- אחרי תגית הסגירה
= `<a href="x">טקסט</a>` — ה-attribute נמצא בתגית הפתיחה. בתגית הסגירה אף פעם לא כותבים attributes.

? כתבת `alt=A sleeping cat` בלי מרכאות. מה יקרה?
- זה יעבוד בדיוק אותו דבר
+ הערך ייחשב `A` בלבד, והשאר ייקרא כ-attributes אחרים
- הדפדפן יציג שגיאה
- כל התגית תתעלם
= רווח מסיים ערך לא ממורכאות. לכן כותבים תמיד מרכאות כפולות.

? רוצה לבטל את `disabled` על שדה קלט. מה עושים?
- כותבים `disabled="false"`
- כותבים `disabled="0"`
+ מוחקים את ה-attribute לגמרי
- כותבים `enabled="true"`
= ב-Boolean Attribute עצם הנוכחות היא הערך. `disabled="false"` עדיין חוסם את השדה.

? מה ההבדל בין `id` ל-`class`?
- `id` ל-CSS ו-`class` ל-JavaScript
+ `id` ייחודי לאלמנט אחד בדף, `class` יכול לחזור על כמה אלמנטים
- `id` לאלמנטים סמנטיים ו-`class` ל-`<div>`
- אין הבדל מעשי
= כלל אצבע: `id` לדבר אחד ויחיד בדף, `class` לכל דבר שחוזר על עצמו.

? רוצה לשמור על אלמנט מידע משלך, שאין לו attribute מוכן ב-HTML. מה הדרך התקנית?
- להמציא attribute בשם כלשהו, למשל `productid="42"`
+ להשתמש ב-`data-`, למשל `data-product-id="42"`
- לשמור את המידע ב-`title`
- לשמור את המידע ב-`class`
= כל attribute שמתחיל ב-`data-` הוא חוקי. שם מומצא יעבוד ברוב הדפדפנים אבל אינו תקני.
```
