...
```quiz
? מה ההבדל בהתנהגות בין `<div>` ל-`<span>`?
+ `<div>` הוא block ומתחיל שורה חדשה, `<span>` הוא inline וזורם בתוך השורה
- `<div>` לטקסט ו-`<span>` לתמונות
- `<div>` סמנטי ו-`<span>` לא
- אין הבדל, אלה שמות נרדפים
= block תופס את כל רוחב השורה. inline תופס רק את רוחב התוכן שלו.

? איזו משמעות סמנטית יש ל-`<div>`?
- הוא מסמן חלק מרכזי בדף
- הוא מסמן קבוצת תוכן קשורה
+ אין לו שום משמעות — הוא מיכל כללי בלבד
- הוא מסמן חלוקה לעמודות
= לכן הכלל: קודם מחפשים אלמנט סמנטי מתאים, ורק אם אין — `<div>`.

? כתבת `<p>לפני <div>בלוק</div> אחרי</p>`. מה יקרה?
- הכול יוצג בתוך הפסקה כרגיל
+ הדפדפן יסגור את הפסקה לפני ה-`<div>`, והמילה ״אחרי״ תישאר מחוצה לה
- הדפדפן יתעלם מה-`<div>`
- תוצג הודעת שגיאה
= `<p>` יכול להכיל רק תוכן inline. הדפדפן מתקן לפי ניחוש, והעץ שנבנה שונה ממה שכתבת.

? רוצה לעטוף את תפריט הניווט של האתר. במה נכון להשתמש?
- `<div class="nav">`
+ `<nav>`
- `<span class="nav">`
- `<section class="nav">`
= זו הדוגמה הקלאסית ל-divitis: `<div>` עובד ונראה זהה, אבל לא מספר לאף אחד מה יש שם.

? מתי נכון להשתמש ב-`<span>`?
- כדי להדגיש טקסט חשוב
+ כשרוצים ידית אחיזה ל-CSS או ל-JavaScript סביב חלק משורת טקסט
- כדי ליצור רווח בין מילים
- כדי להחליף `<p>` בטקסט קצר
= להדגשה עם משמעות יש `<strong>` ו-`<em>`. `<span>` מיועד למקרים שבהם אין משמעות מתאימה.
```
