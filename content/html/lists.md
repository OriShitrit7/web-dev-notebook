...
```quiz
? מתי בוחרים `<ol>` ולא `<ul>`?
- כשיש יותר מחמישה פריטים
+ כשלסדר הפריטים יש משמעות, כמו שלבים במתכון
- כשרוצים מספרים במקום נקודות מסיבות עיצוב
- כשהרשימה מקוננת
= המבחן: אם אערבב את הסדר, משהו יישבר? אם כן — `<ol>`. אם לא — `<ul>`.

? רוצה שהפריט הראשון ברשימה ממוספרת יתחיל מ-5. מה עושים?
- כותבים `<li>5. הפריט</li>`
+ כותבים `<ol start="5">`
- כותבים `<ol type="5">`
- אי אפשר, המספור תמיד מתחיל מ-1
= הדפדפן ממספר לבד. מספור ידני ייצור ״5. 1.״ וגם יישבר כשיתווסף פריט.

? איפה צריכה להיכנס רשימה מקוננת?
- בין שני אלמנטי `<li>`
+ בתוך ה-`<li>` שאליו היא שייכת
- ישירות בתוך ה-`<ul>` אחרי כל הפריטים
- מחוץ לרשימה, אחרי תגית הסגירה
= שתי הצורות נראות כמעט זהות בדפדפן, אבל רק בצורה הנכונה נוצר הקשר בין הפריט לתת-הפריטים.

? מה מותר להיות Child ישיר של `<ul>`?
- כל אלמנט
+ רק `<li>`
- רק `<li>` ו-`<p>`
- רק טקסט
= תוכן עשיר מותר — פשוט צריך להיות בתוך ה-`<li>` ולא ישירות ב-`<ul>`.

? איזו רשימה מתאימה למילון מונחים והגדרות?
- `<ul>` עם `<strong>` לכל מונח
- `<ol>` ממוספרת
+ `<dl>` עם `<dt>` למונח ו-`<dd>` להסבר
- טבלה עם שתי עמודות
= `<dl>` היא המבנה היחיד מהשלושה שלא משתמש ב-`<li>` בכלל.
```
