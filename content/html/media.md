...
```quiz
? מה ההבדל בין `alt=""` ריק לבין `<img>` בלי `alt` בכלל?
- אין הבדל, שניהם זהים
+ `alt=""` אומר ״תמונה דקורטיבית, דלג״, ובלי `alt` קורא מסך יקריא את שם הקובץ
- `alt=""` אינו תקני
- בלי `alt` התמונה לא תיטען
= לכן לכל `<img>` צריך `alt` — גם אם הוא ריק. זו הדרך לומר במפורש שאין מה להקריא.

? למה כדאי לכתוב `width` ו-`height` על `<img>` גם אם מעצבים ב-CSS?
- כדי שהתמונה תיטען מהר יותר
+ כדי שהדפדפן ישמור מראש את השטח והדף לא יקפוץ כשהתמונה נטענת
- כי אחרת התמונה תוצג בגודל מלא
- כי CSS לא יכול לשנות גודל תמונות
= התופעה נקראת layout shift — הסיבה לכך שלפעמים לוחצים על כפתור ובדיוק אז הוא זז.

? הוספת `<video src="clip.mp4">` ולא רואים כלום בדף. מה כנראה חסר?
- `width` ו-`height`
+ `controls` — בלעדיו הנגן לא מוצג
- `poster`
- `autoplay`
= `controls` הוא Boolean Attribute שמציג את כפתורי ההפעלה. בלעדיו אין ממשק.

? למה `autoplay` לבדו בדרך כלל לא עובד?
- כי הוא הוסר מהתקן
+ כי דפדפנים חוסמים ניגון אוטומטי עם קול — צריך גם `muted`
- כי הוא עובד רק ב-`<audio>`
- כי צריך להוסיף גם `loop`
= זו הייתה אחת ההתנהגויות השנואות באינטרנט, ולכן הדפדפנים חוסמים אותה.

? תמונה נמצאת בתוך `<a>`. מה צריך לכתוב ב-`alt`?
- תיאור של מה שרואים בתמונה
+ לאן הקישור מוביל
- להשאיר ריק כי זו תמונה דקורטיבית
- את שם קובץ התמונה
= למשתמש קורא מסך זה הקישור היחיד שיש. ״הרים בזריחה״ לא יאמר לו לאן ילחץ.
```
