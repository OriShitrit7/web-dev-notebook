...
```quiz
? כתבת `<p>שלום      עולם</p>` עם שישה רווחים. מה יוצג?
- שישה רווחים, בדיוק כפי שנכתב
+ רווח אחד — הדפדפן מכווץ רצפי רווחים
- הדפדפן יציג שגיאה
- הטקסט יישבר לשתי שורות
= זה נקרא whitespace collapsing, וזו הסיבה שאפשר להזיח קוד HTML בלי לשנות את התוצאה.

? מתי נכון להשתמש ב-`<b>` ולא ב-`<strong>`?
- תמיד — הם זהים
+ כשהטקסט בולט ויזואלית בלי חשיבות נוספת, למשל שם מוצר
- כשרוצים הדגשה חזקה יותר
- אף פעם, `<b>` הוסר מהתקן
= שניהם נראים זהים. `<strong>` מסמן חשיבות, `<b>` רק בליטוּת ויזואלית. בספק — `<strong>`.

? רוצה רווח בין שני חלקי טקסט. מה הדרך הנכונה?
- `<br><br>` בין החלקים
+ שתי פסקאות `<p>` נפרדות
- כמה רווחים ברצף
- `<hr>` בין החלקים
= `<br><br>` ייצור רווח, אבל מבחינת המבנה זו עדיין פסקה אחת שהתוכן שלה מתערבב.

? כתבת `<q>שלום</q>`. מה יוצג?
- שלום, בלי שום סימן
+ שלום במרכאות — הדפדפן מוסיף אותן לבד
- שלום בשורה נפרדת ומוזחת
- שלום בגופן נטוי
= זה תפקידו של `<q>`. אם נוסיף מרכאות ידנית, יופיעו שתי שכבות של מרכאות.

? למה חייבים לכתוב `&lt;` כדי להציג את התו `<` בטקסט?
- כדי שהטקסט ייראה יפה יותר
+ כי אחרת הדפדפן יחשוב שמתחילה תגית ויבלע את הטקסט
- כי `<` אינו קיים בקידוד UTF-8
- כי זה נדרש רק בעברית
= שלושת ה-entities החיוניים הם `&lt;`, `&gt;` ו-`&amp;` — התווים שיש להם משמעות תחבירית.
```
