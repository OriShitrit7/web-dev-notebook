...
```quiz
? הקובץ `pages/contact.html` צריך להציג את `images/logo.png` שבשורש. מה כותבים?
- `images/logo.png`
+ `../images/logo.png`
- `./images/logo.png`
- `/pages/images/logo.png`
= הקובץ יושב בתוך תיקייה, ולכן צריך קודם לצאת החוצה עם `../` ואז להיכנס ל-images.

? מה המשמעות של נתיב שמתחיל בלוכסן, כמו `/images/logo.png`?
- התיקייה הנוכחית
+ שורש הדומיין — לא תיקיית הפרויקט
- תיקיית האב
- נתיב במחשב המקומי
= זו בדיוק הסיבה שהוא נשבר ב-GitHub Pages כשהאתר יושב בתת-תיקייה.

? תמונה מוצגת מצוין בשרת המקומי ונעלמת אחרי העלאה ל-GitHub Pages. מה הכי סביר?
- הקובץ לא הועלה
+ נתיב מהשורש, או אות גדולה בשם הקובץ שה-Mac מתעלם ממנה ושרת Linux לא
- הדפדפן לא תומך בפורמט
- חסר `alt`
= שתי התקלות האלה משותפות במשהו: שתיהן לא קיימות במחשב שלך ומופיעות רק אחרי ההעלאה.

? למה `my photo.jpg` הוא שם קובץ בעייתי?
- כי הוא ארוך מדי
+ כי הרווח הופך ל-`%20` בכתובת, והשם נשבר בנתיב לא ממורכא
- כי אסור אותיות קטנות בשמות קבצים
- כי `.jpg` אינו נתמך
= הנוהג: אותיות קטנות, מקפים במקום רווחים, ובלי עברית ותווים מיוחדים.

? הקלדת נתיב שגוי ב-`src` של תמונה. מה תראי בדף?
- הודעת שגיאה אדומה
+ את הטקסט מה-`alt` במקום התמונה — בלי שום אזהרה בדף
- ריבוע שחור
- הדף לא ייטען כלל
= נתיב שבור נכשל בשקט. לכן נתיבים שבורים שורדים כל כך הרבה זמן בלי שמישהו שם לב.
```
