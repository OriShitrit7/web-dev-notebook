...
```quiz
? כתבת `<a>לחצו כאן</a>` בלי `href`. מה יקרה?
- הקישור יוביל לראש הדף
+ זה לא יהיה קישור כלל — בלי צבע, בלי לחיצה ובלי גישה ממקלדת
- הדפדפן יציג שגיאה
- הקישור יעבוד אבל לא ישנה כתובת
= `href` הוא מה שהופך את `<a>` לקישור. בלעדיו זה טקסט רגיל.

? רוצה לקשר לסעיף בתוך אותו דף. מה צריך?
- רק `<a href="#contact">`
+ `<a href="#contact">` בקישור, ו-`id="contact"` ביעד
- `<a href="contact">` ו-`name="contact"` ביעד
- `<a href="#contact">` ו-`id="#contact"` ביעד
= צריך שני צדדים שמתאימים. הסולמית מופיעה רק ב-`href`, אף פעם לא בערך של ה-`id`.

? כתבת `<a href="example.com">`. לאן זה יוביל?
- לאתר example.com
+ לקובץ בשם example.com באתר שלך — וכנראה ל-404
- לשום מקום, הקישור לא יעבוד
- ל-example.com אבל בלי HTTPS
= בלי `https://` הדפדפן מפרש את הערך כנתיב יחסי בתוך האתר הנוכחי.

? למה ״לחצו כאן״ הוא טקסט קישור גרוע?
- כי הוא ארוך מדי
+ כי משתמשי קורא מסך רואים רשימת קישורים בלי הטקסט שסביבם
- כי גוגל מענישה על ניסוח כזה
- כי הוא לא עובד בנייד
= המבחן: אם תקראי רק את טקסט הקישור, בלי שום דבר סביבו — תביני לאן הוא מוביל?

? מה נכון לגבי `target="_blank"`?
- הוא מומלץ לכל קישור חיצוני
+ הוא מבטל למשתמש את כפתור ״חזור״, ולכן כדאי לציין בטקסט שנפתחת לשונית
- הוא הוסר מהתקן
- הוא פותח את הקישור בחלון קופץ
= בנוסף נהוג להוסיף `rel="noopener"`, אף שדפדפנים מודרניים כבר מחילים אותו לבד.
```
