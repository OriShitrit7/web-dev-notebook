...
```quiz
? כמה אלמנטי `<main>` מותר שיהיו בדף?
+ אחד בלבד
- אחד לכל `<section>`
- כמה שרוצים
- שניים לכל היותר
= `<main>` הוא היחיד בקבוצה שמוגבל לאחד, ואסור לו להיות בתוך `<article>`, `<header>`, `<footer>` או `<nav>`.

? האם מותר `<header>` בתוך `<article>`?
- לא, `<header>` שייך רק לראש הדף
+ כן — `<header>` ו-`<footer>` יחסיים לחלק שבתוכו הם נמצאים
- רק אם אין `<header>` אחר בדף
- רק אם ה-`<article>` נמצא בתוך `<main>`
= ל-`<article>` יכול להיות `<header>` עם הכותרת והתאריך, ו-`<footer>` עם התגיות.

? פוסט בבלוג שאפשר לשתף בפני עצמו — איזה אלמנט מתאים?
- `<section>`
+ `<article>`
- `<aside>`
- `<div>`
= המבחן: אם אשלח רק את החלק הזה בהודעה, הוא יעמוד בפני עצמו? אם כן — `<article>`.

? הוספת `<header>`, `<nav>` ו-`<main>` במקום `<div>`, והדף נראה בדיוק אותו דבר. למה?
- כנראה יש שגיאת תחביר
+ לאלמנטים סמנטיים אין עיצוב ברירת מחדל — הם משנים משמעות בלבד
- צריך להוסיף להם `class` כדי שיפעלו
- הם עובדים רק אחרי שמוסיפים CSS
= זו בדיוק הנקודה, ובדיוק הסיבה שקל לוותר עליהם בלי לשים לב.

? מה המשתמש מרוויח מדף שבנוי סמנטית ולא מ-`<div>`?
- הדף נטען מהר יותר
- הדף נראה טוב יותר בלי CSS
+ קורא מסך מקבל נקודות ציון שאפשר לקפוץ ביניהן, למשל ישר ל-main
- הדף עובד בדפדפנים ישנים
= בלי אלמנטים סמנטיים אין landmarks, והמשתמש נאלץ להאזין לכל התפריט בכל דף מחדש.
```
