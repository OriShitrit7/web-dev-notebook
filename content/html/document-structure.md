...
```quiz
? מה התפקיד של `<!DOCTYPE html>`?
- להגדיר את שפת הדף
+ לומר לדפדפן לקרוא את הקובץ לפי התקן המודרני
- לפתוח את אלמנט השורש של המסמך
- לטעון את קובץ ה-CSS
= זו הצהרה ולא תגית. בלעדיה הדפדפן עובר ל-Quirks Mode, ומידות ב-CSS מתנהגות אחרת.

? דף נטען ונראה תקין, אבל מרווחים ב-CSS מתנהגים באופן מוזר. מה כדאי לבדוק קודם?
- שה-`<title>` קיים
+ שהשורה `<!DOCTYPE html>` נמצאת בראש הקובץ
- שה-`<body>` נסגר כראוי
- שקובץ ה-CSS נטען
= זהו הסימפטום הקלאסי של Quirks Mode. הדף לא נשבר בגלוי, ולכן קשה לאתר את הסיבה.

? כמה אלמנטי `<body>` מותר שיהיו במסמך HTML אחד?
+ אחד בדיוק
- אחד לכל `<section>`
- כמה שרוצים
- שניים — אחד לתוכן ואחד לסקריפטים
= בתוך `<html>` יש בדיוק `<head>` אחד ו-`<body>` אחד, ותמיד בסדר הזה.

? כתבת `<p>טקסט</p>` בטעות בתוך `<head>`. מה יקרה?
- הדפדפן יציג שגיאה ויעצור
- הפסקה תוצג בתוך ה-head ולא תיראה
+ הדפדפן יסגור את ה-head לבד ויעביר את הפסקה ל-body
- הפסקה תימחק לגמרי
= הדפדפן מתקן HTML שבור לפי ניחוש. הפסקה תוצג, אבל העץ שייבנה שונה ממה שכתבת.

? מה ההבדל בין `lang="he"` לבין `dir="rtl"`?
- אין הבדל, שניהם מגדירים עברית
+ `lang` מגדיר את שפת הדף, ו-`dir` את כיוון הכתיבה והיישור
- `lang` לדפדפן ו-`dir` למנועי חיפוש
- `dir` מיותר אם כבר כתבנו `lang="he"`
= בלי `dir="rtl"` הטקסט העברי עדיין ייקרא נכון, אבל היישור וסדר הפיסוק יישברו.
```
